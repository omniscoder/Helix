...
def main() -> None:
    print("=== DNA SUMMARY ===")
    dna = helix_api.dna_summary(input_path="input/dna/plasmid_demo.fna", k=4, max_diff=1, window=100, step=25)
    print(f"Length: {dna['length']} nt | GC content: {dna['gc_content']*100:.2f}%")
    print(f"Detected {len(dna['kmer_clusters'])} k-mer clusters")

    print("\n=== TRIAGE REPORT ===")
    triage_report = helix_api.triage_report(input_path="input/dna/plasmid_demo.fna", k=4, max_diff=1, min_orf_length=60)
    ...
    if helix_api.PROTEIN_AVAILABLE:
        print("\n=== PROTEIN SUMMARY ===")
        protein = helix_api.protein_summary(input_path="input/protein/demo_protein.faa", window=11)
